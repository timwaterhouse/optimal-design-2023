...
myBib <- ReadBib(here("slides/refs.bib"), check = FALSE)
```
...
## Tim Waterhouse & Kyle Baron <br/> Metrum Research Group
## Julie Bertrand & Romain Leroux<br/> Université de Paris, INSERM, IAME
## 28 March, 2023
...
* $\mathbf{\Psi}$ is the vector of population parameters (e.g., `THETA`s, `OMEGA`s, and `SIGMA`s in NONMEM),
...
.center[<img src="https://whatemoji.org/wp-content/uploads/2020/07/Smiling-Face-With-Sunglasses-Emoji.png" width="200">]
...
.center[
<img src="pics/nonmem-design-webinar.png" alt="" title="" width="70%"  style="border: 3px solid red"/>
]

.footnote[[Recording: https://sites.google.com/view/mcssig/mcs-sig#h.v7bdq6l7d86v](https://sites.google.com/view/mcssig/mcs-sig#h.v7bdq6l7d86v)]
...
.pull-left[
```{r echo=F}
knitr::include_graphics("pics/pfim2.png")
```
]
...
.footnote[`r Citet(myBib, "Leroux2022")`]
...
.font-90[
```{r echo=FALSE}
output <- readRDS(here("script/opt1.rds"))
summary(output)
```
]
]
...
# `PopED`: Another post dose sample at SS
...
# `PopED`: *D*-optimal design: Another post dose sample at SS
...
.font-90[
```{r echo=FALSE}
output_extra_ss <- readRDS(here("script/opt2.rds"))
summary(output_extra_ss)
```
]]
...
.font-90[
```{r echo=FALSE}
output_final <- readRDS(here("script/opt3.rds"))
summary(output_final)
```
]]
...
est <- readRDS(here("script/ex1_est2.rds"))
plot_est(est)
```
]
...
.pull-right[
![](pics/pfim-ex1-fig-profile.png)
]

???

`PFIM` includes a function to generate a quick test plot showing the typical response(s), along with the initial sampling times.
...
****************************  design1  ******************************* 
   ...
   Arm_name Response tau Tinf                        Time_dose                                            Amount_dose          Sampling_times Number_of_subjects
1 Bras test  RespPK1   -    - 0, 24, 48, 72, 96, 120, 144, 168 10000, 10000, 10000, 10000, 10000, 10000, 10000, 10000 (5, 120, 144, 168, 336)                 12
```
...
**** Fixed effect 
             ...
             μ_Cl        μ_ka          μ_V
μ_Cl  0.594311668  -0.8211656 -0.002425272
μ_ka -0.821165609 564.9226170 -2.218057475
...
**** Variance components 
                      ...
                      ω²_Cl       ω²_ka        ω²_V σ_inter_RespPK1 σ_slope_RespPK1
ω²_Cl           147.1693161   0.1756023   0.2450810       0.1067300        7.591955
ω²_ka             0.1756023  51.9429628 128.1192438       0.2065110       21.923776
ω²_V              0.2450810 128.1192438 319.9281155       0.4876588       51.107443
σ_inter_RespPK1   0.1067300   0.2065110   0.4876588      34.2620161      238.748328
σ_slope_RespPK1   7.5919547  21.9237756  51.1074432     238.7483285     5561.894327
```
...
**** Variance components 
                       ...
                       ω²_Cl        ω²_ka         ω²_V σ_inter_RespPK1 σ_slope_RespPK1
ω²_Cl            1.000000000 -0.007756317  0.007627033     0.003569392    -0.008783515
ω²_ka           -0.007756317  1.000000000 -0.993851261     0.013130503    -0.027650113
ω²_V             0.007627033 -0.993851261  1.000000000    -0.010893393     0.022752548
σ_inter_RespPK1  0.003569392  0.013130503 -0.010893393     1.000000000    -0.547261481
σ_slope_RespPK1 -0.008783515 -0.027650113  0.022752548    -0.547261481     1.000000000
```
...
**********
                 ...
                 Value           SE      RSE (%)
μ_Cl             10.00 5.252748e+03  52527.47704
μ_ka              0.25 2.172884e+03 869153.72640
μ_V             100.00 5.514727e+05 551472.68815
ω²_Cl             0.20 8.243712e-02     41.21856
ω²_ka             0.08 1.254450e+00   1568.06291
ω²_V              0.10 5.054008e-01    505.40077
σ_inter_RespPK1   1.00 2.041242e-01     20.41242
σ_slope_RespPK1   0.05 1.603788e-02     32.07577
```
...
4.98139743936754, 119.999815287607, 143.000000389577,
...
**********
                 ...
                 Value          SE   RSE (%)
μ_Cl             10.00  1.38874526  13.88745
μ_ka              0.25  0.22079467  88.31787
μ_V             100.00 56.80758006  56.80758
ω²_Cl             0.20  0.08241346  41.20673
ω²_ka             0.08  0.48045173 600.56467
ω²_V              0.10  0.19874160 198.74160
σ_inter_RespPK1   1.00  0.20408418  20.40842
σ_slope_RespPK1   0.05  0.01571215  31.42429
```
]
]
...
# `PFIM`: *D*-optimal design: Another post dose sample at SS
...
brasTestDesign1 <- addSampling(
  ...
  SamplingTimes(
    outcome = "RespPK1",
    sample_time = c(5, c(120, 144, 168), 172, 336)
  )
)
samplingBoundsConstraint <- SamplingConstraint(
  response = "RespPK",
...
5.99998869005838, 119.002565130602, 143.972085758471,
167.031903767095, 169.343414960844, 286.225935672166                 12
...
**********
                 ...
                 Value          SE  RSE (%)
μ_Cl             10.00  1.29838751 12.98388
μ_ka              0.25  0.02665015 10.66006
μ_V             100.00 10.80456819 10.80457
ω²_Cl             0.20  0.08221782 41.10891
ω²_ka             0.08  0.05033867 62.92333
ω²_V              0.10  0.05118174 51.18174
σ_inter_RespPK1   1.00  0.19844858 19.84486
σ_slope_RespPK1   0.05  0.01098263 21.96525
```
]
]
...
4.52576291331019, 119.000000458829, 143.000791500247,
169.416690394867, 205.731885951118, 331.266297788294
...
**********
                 ...
                 Value          SE  RSE (%)
μ_Cl             10.00  1.29952262 12.99523
μ_ka              0.25  0.02873640 11.49456
μ_V             100.00 10.96078787 10.96079
ω²_Cl             0.20  0.08235915 41.17958
ω²_ka             0.08  0.05679475 70.99344
ω²_V              0.10  0.05102656 51.02656
σ_inter_RespPK1   1.00  0.18241440 18.24144
σ_slope_RespPK1   0.05  0.01236994 24.73988
```
]
]
...
```{r echo=FALSE}
mod <- mread(here("script/model/model_poped.mod"))
see(mod)
```
...
p <- readRDS(here("script/windows_mrg.rds"))
p
```
]
...
.pull-left[
```{r echo=FALSE, fig.height=4, fig.width=4, out.width='100%'}
est <- readRDS(here("script/ex2_est1.rds"))
plot_est(est)
```
]
...
.center[<img src="pics/pfim-ex2-fig-profile.png" width="700">]
...
**********
               ...
               Value           SE      RSE (%)
μ_CL            0.50 0.0384775981  7.695519616
μ_KM            1.20 0.0196656516  1.638804302
μ_Q            10.00 0.0005779689  0.005779689
μ_V1            2.50 0.1498846589  5.995386356
μ_V2            4.00 0.1009500500  2.523751249
μ_VMAX         20.00 1.8703324957  9.351662478
ω²_CL           0.20 0.0497454303 24.872715150
ω²_V1           0.10 0.0246396407 24.639640672
ω²_VMAX         0.20 0.0603291975 30.164598760
σ_slope_RespPK  0.15 0.0085116372  5.674424799
```
...
mod_gcsf <- mread(here("script/model/gcsf_poped.mod"))
doses <- c(0.3, 1, 3, 10, 30)
...
poped_db_gcsf <- create.poped.database(
  ...
  discrete_a = list(DOSE = c(0.3, 1, 3, 10, 30), WT = 75, TAU = 24)
)
```
...
```{r echo=FALSE}
FIM_gcsf <- readRDS(here("script/FIM_gcsf.rds"))
get_rse(FIM_gcsf, poped_db_gcsf)
```
...
```{r echo=FALSE}
rse_gcsf <- readRDS(here("script/rse_gcsf.rds"))
as.data.frame(bind_cols(design = unique(designs[["design"]]), rse_gcsf)) %>% 
  mutate_if(is.numeric, round, 1) %>% 
  t() %>% 
  as_tibble(rownames = "Parameter") %>% 
  set_names(c("Parameter", .[1,2:4])) %>% 
  slice(-1) %>% 
  dplyr::filter(Parameter %in% c("SC1", "SM1", "SM2")) %>% 
  knitr::kable(format = "html") %>% 
