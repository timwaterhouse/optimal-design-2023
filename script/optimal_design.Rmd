...
myBib <- ReadBib(here::here("slides/refs.bib"), check = FALSE)
```
...
saveRDS(output, here("script/opt1.rds"))
```
...
saveRDS(output_extra_ss, here("script/opt2.rds"))
```
...
saveRDS(output_final, here("script/opt3.rds"))
```
...
The next function performs 2 steps for each replicate: (1) take the template control stream and adjusts it to point to the relevant dataset and set the random seed; and (2) runs the model using `bbr`.  Additional info on the use and functionality of `bbr` is available at https://metrumresearchgroup.github.io/bbr/.
...
Set up bbr to run nonmem - we also need the bbi executable. You can run  to install onto your system.
...
may not map 1:1 to windows usage. Check https://metrumresearchgroup.github.io/bbr/ for more information.

```
# once per machine:
bbr::use_bbi()

# create bbi.yaml in model directory - this should only need to get run once per new project/directory
bbi_init(
  .dir = design_dir,           # your modeling directory
  .nonmem_dir = "/opt/NONMEM", # directory where NONMEM is installed
  .nonmem_version = "nm75"   # default version of NONMEM to use
)
```
...
saveRDS(est, here("script/ex1_est1.rds"))
```

```{r}
est <- readRDS(here("script/ex1_est1.rds"))
...
```{r eval=FALSE}
est <- get_est(design_dir)
saveRDS(est, here("script/ex1_est2.rds"))
```
...
saveRDS(p, here("script/windows_mrg.rds"))
```
...
```{r eval=FALSE}
est <- get_est(design_dir)
saveRDS(est, here("script/ex2_est1.rds"))
```
...
```{r}
mod_gcsf <- mread(here("script/model/gcsf_poped.mod")) %>% zero_re()
...
saveRDS(FIM_gcsf, here("script/FIM_gcsf.rds"))
```
...
saveRDS(rse_gcsf, here("script/rse_gcsf.rds"))
```
